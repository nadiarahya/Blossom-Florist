    ...
    if (!isset($_SESSION['liked'][$id])) {
        mysqli_query(
            ...
            "INSERT INTO product_likes(product_id,user_id) 
             VALUES($id," . (isset($_SESSION['user_id']) ? (int) $_SESSION['user_id'] : 'NULL') . ")"
        );
...
if (!$p) {
    echo '<div class="empty">Produk tidak ditemukan.</div>';
    ...
    <div class="card">
        ...
        <div class="hero-actions">
            ...
            <form method="post" class="inline">
                <button class="btn secondary" name="like" value="1">
                    Like Produk
                </button>
            </form>
        </div>
        ...
        if (mysqli_num_rows($reviews) == 0) {
            echo '<p class="muted">Belum ada review. Jadilah yang pertama.</p>';
        }

        while ($r = mysqli_fetch_assoc($reviews)):
        ?>
            <div class="review-box">
                <b><?= e($r['reviewer_name']); ?></b>
                —
